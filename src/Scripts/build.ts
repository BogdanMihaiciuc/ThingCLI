...
export async function build(): Promise<string[]> {
     ...
     */
     async function buildProject(projectName: string, path: string, outPath: string): Promise<void> {
        ...
        const { tsDiagnostics, validationDiagnostics } = firePostTransformActions(program, twConfig.store, projectName, emitResult);
        ...
        process.stdout.write(`\r\x1b[1;32m✔\x1b[0m Built ${projectName || 'project'}${validationDiagnostics.length ? ' (with warnings):' : '     '}\n`);
        ...
        for (const message of validationDiagnostics) {
